...
@interface QChatViewController ()<UIAlertViewDelegate,EMClientDelegate>
{
               ...
               selectAtTarget:(EaseSelectAtTargetCallback)selectedCallback
{
    ...
    if (chatGroup) {
        if (!chatGroup.occupants) {
            __weak QChatViewController* weakSelf = self;
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    __strong QChatViewController *strongSelf = weakSelf;
                    if (strongSelf) {
                        [strongSelf hideHud];
                        if (error) {
                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    if (model.isSender) {
        model.avatarURLPath = [QAliyunOSS avatarUrl:[QUser current].id.stringValue].absoluteString;
    }else{
        QChatUsers *chatUser = [QChatUsers seachWithUUID:message.conversationId];
        if (chatUser) {
...
- (void)deleteAllMessages:(id)sender
{
    ...
    if ([sender isKindOfClass:[NSNotification class]]) {
        ...
        [MBProgressHUD showTip:@"please make sure to delete"];
    }
}
